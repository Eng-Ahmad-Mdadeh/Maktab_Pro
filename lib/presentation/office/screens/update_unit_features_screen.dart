...
class UpdateUnitFeaturesScreen extends StatelessWidget {
  const UpdateUnitFeaturesScreen({super.key, required this.unit});
      ...
      child: BlocConsumer<UnitBloc, UnitState>(
        listener: (context, state) {
          if (state.unitApiCallState == UnitApiCallState.noCall) {
            ...
            context.pop();
            MaktabSnackbar.showSuccess(context, 'تم تعديل مميزات الوحدة بنجاح');
          } else if (state.unitApiCallState == UnitApiCallState.failure) {
            MaktabSnackbar.showError(context, 'فشل تعديل المميزات');
          }
        },
        builder: (context, state) => state.isInitialized
            ? Scaffold(
                appBar: const MaktabAppBar(title: 'تعديل مميزات الوحدة'),
                body: SafeArea(
