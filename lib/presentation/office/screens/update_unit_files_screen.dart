...
class UpdateUnitFilesScreen extends StatelessWidget {
  const UpdateUnitFilesScreen({super.key, required this.unit});
    ...
    return BlocProvider.value(
      ...
      child: BlocConsumer<UnitBloc, UnitState>(
        listenWhen: (previous, current) =>
            previous.unitApiCallState != current.unitApiCallState,
        listener: (context, state) {
          if (state.unitApiCallState == UnitApiCallState.noCall) {
            ...
            context.pop();
            MaktabSnackbar.showSuccess(context, 'تم تعديل ملفات الوحدة بنجاح');
          } else if (state.unitApiCallState == UnitApiCallState.failure) {
            MaktabSnackbar.showError(context, 'فشل تعديل الملفات');
          }
        },
        builder: (context, state) => state.isInitialized
            ? Scaffold(
                appBar: const MaktabAppBar(title: 'تعديل ملفات الوحدة'),
                body: SafeArea(
