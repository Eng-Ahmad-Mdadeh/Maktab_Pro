...
import 'package:maktab_lessor/presentation/office/widgets/unit_facilities_grid_view.dart';
...
class UpdateUnitFacilitiesScreen extends StatelessWidget {
  const UpdateUnitFacilitiesScreen({super.key, required this.unit});
      ...
      child: BlocConsumer<UnitBloc, UnitState>(
        listener: (context, state) {
          if (state.unitApiCallState == UnitApiCallState.noCall) {
            ...
            context.pop();
            MaktabSnackbar.showSuccess(context, 'تم تعديل مرافق الوحدة بنجاح');
          } else if (state.unitApiCallState == UnitApiCallState.failure) {
            MaktabSnackbar.showError(context, 'فشل تعديل المرافق');
          }
        },
        builder: (context, state) => state.isInitialized
            ? Scaffold(
                appBar: const MaktabAppBar(title: 'تعديل مرافق الوحدة'),
                body: SafeArea(
