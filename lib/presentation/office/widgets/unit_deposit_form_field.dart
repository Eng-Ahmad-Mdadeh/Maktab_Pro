...
class UnitDepositFormField extends StatefulWidget {
  const UnitDepositFormField({super.key});

  @override
  State<UnitDepositFormField> createState() => _UnitDepositFormFieldState();
}

class _UnitDepositFormFieldState extends State<UnitDepositFormField> {
  ...
  void initState() {
    _depositController = TextEditingController(
        text: context.read<UnitBloc>().state.depositAmount > -1
            ? context.read<UnitBloc>().state.depositAmount.toString()
            : '');
    super.initState();
  }
                    ...
                    return MaktabTextFormField(
                      ...
                      validator: (value) {
                        if (value!.trim().isEmpty) {
                          context
                              .read<UnitBloc>()
                              .add(const ChangeDepositAmountEvent(''));
                          return '';
                        } else {
                          context
                              .read<UnitBloc>()
