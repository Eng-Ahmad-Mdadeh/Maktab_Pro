class VersionHelper implements Comparable<VersionHelper> {
  ...
  VersionHelper(this.major, this.minor, this.patch,
  ...
  @override
  int compareTo(VersionHelper? other) {
    if (other == null) {
  ...
  VersionHelper incrementMajor() => VersionHelper(major + 1, 0, 0);

  VersionHelper incrementMinor() => VersionHelper(major, minor + 1, 0);

  VersionHelper incrementPatch() => VersionHelper(major, minor, patch + 1);

  VersionHelper incrementPreRelease() {
    if (!isPreRelease) {
      throw Exception(
    ...
    for (var i = newPreRelease.length - 1; i >= 0; i--) {
      ...
      if (VersionHelper._isNumeric(segment)) {
  ...
  static VersionHelper parse(String versionString) {
    ...
    return VersionHelper(major, minor ?? 0, patch ?? 0,
        build: build, preRelease: preReleaseList);
  }

  static int _compare(VersionHelper? a, VersionHelper? b) {
    if (a == null) {
